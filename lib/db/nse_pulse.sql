-- Market Pulse AI — full database schema for a fresh live server.
-- Import via phpMyAdmin (Import tab) or: mysql -u USER -p < nse_pulse.sql
--
-- Creates the `nse_pulse` database, all 12 tables, and seeds the default
-- roles/permissions. The API server creates the first admin account on boot
-- from ADMIN_EMAIL / ADMIN_PASSWORD in .env.

CREATE DATABASE IF NOT EXISTS `nse_pulse`
...
-- admin → all permissions; user → dashboard/watchlist/portfolio/settings
